// Account service — Express bootstrap, DB + Redis startup, routes, error handler, port 3001
...
// ── API routes (account-service receives requests from api-gateway)
...
async function start(): Promise<void> {
  ...
  if (!redisOk) {
    logger.warn('redis_unavailable', { message: 'Account service starting without Redis' });
  }

  const port = parseInt(process.env.ACCOUNT_SERVICE_PORT || '3001', 10);
  app.listen(port, () => {
    logger.info('server_started', { service: 'account-service', port });
  });
}
